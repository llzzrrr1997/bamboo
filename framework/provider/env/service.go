...
// BambooEnv 是 Env 的具体实现
type BambooEnv struct {
	folder string            // 代表.env所在的目录
	maps   map[string]string // 保存所有的环境变量
}

// NewBambooEnv 有一个参数，.env文件所在的目录
// example: NewBambooEnv("/envfolder/") 会读取文件: /envfolder/.env
// .env的文件格式 FOO_ENV=BAR
func NewBambooEnv(params ...interface{}) (interface{}, error) {
	if len(params) != 1 {
		return nil, errors.New("NewBambooEnv param error")
	}
	...
	for _, e := range os.Environ() {
		...
		bambooEnv.maps[pair[0]] = pair[1]
	}

	// 返回实例
	return bambooEnv, nil
}

// AppEnv 获取表示当前APP环境的变量APP_ENV
func (en *BambooEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *BambooEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *BambooEnv) Get(key string) string {
...
func (en *BambooEnv) All() map[string]string {
	return en.maps
}
