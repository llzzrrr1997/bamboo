...
type BambooTraceService struct {
...
func NewBambooTraceService(params ...interface{}) (interface{}, error) {
	c := params[0].(framework.Container)
	idService := c.MustMake(contract.IDKey).(contract.IDService)
	return &BambooTraceService{idService: idService}, nil
}

// WithTrace register new trace to context
func (t *BambooTraceService) WithTrace(c context.Context, trace *contract.TraceContext) context.Context {
...
func (t *BambooTraceService) GetTrace(c context.Context) *contract.TraceContext {
	if ginC, ok := c.(*gin.Context); ok {
...
func (t *BambooTraceService) NewTrace() *contract.TraceContext {
...
func (t *BambooTraceService) StartSpan(tc *contract.TraceContext) *contract.TraceContext {
...
// GetTrace By Http
func (t *BambooTraceService) ExtractHTTP(req *http.Request) *contract.TraceContext {
	tc := &contract.TraceContext{}
...
// Set Trace to Http
func (t *BambooTraceService) InjectHTTP(req *http.Request, tc *contract.TraceContext) *http.Request {
...
func (t *BambooTraceService) ToMap(tc *contract.TraceContext) map[string]string {
	m := map[string]string{}
	if tc == nil {
		return m
	}
...
// func (t *BambooTraceService) SetTraceIDService(service contract.IDService) {
// 	t.traceIDGenerator = service
// }

// func (t *BambooTraceService) SetSpanIDService(service contract.IDService) {
