...
import (
	...
	"github.com/llzzrrr1997/bamboo/framework/provider/log/services"
	"io"
	"strings"
)

// BambooLogServiceProvider 服务提供者
type BambooLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *BambooLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewBambooConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewBambooSingleLog
	case "rotate":
		return services.NewBambooRotateLog
	case "console":
		return services.NewBambooConsoleLog
	case "custom":
		return services.NewBambooCustomLog
	default:
		return services.NewBambooConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *BambooLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *BambooLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *BambooLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *BambooLogServiceProvider) Name() string {
